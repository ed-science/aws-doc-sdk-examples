...
def check_files(root, file_filter, quiet, full, warn=True):
    ...
    for path, dirs, files in os.walk(root):
        ...
        for filename in fnmatch.filter(files, file_filter):
            ...
            errors = [snippet_start_check(words, file_path)]
            if full:
                errors.extend(
                    (
                        snippet_author_check(snippets, warn),
                        snippet_service_check(snippets, warn),
                        snippet_description_check(snippets, warn),
                        snippet_type_check(snippets, warn),
                        snippet_date_check(snippets, warn),
                        snippet_keyword_check(snippets, warn),
                    )
                )
            if not quiet:
            ...
            if warn:
                ...
                for error in errors:
                    if error:
                        print(error)
    print(f"{str(filecount)} files scanned in {root}")
    print("")
...
def verify_no_deny_list_words(file_contents, file_location):
    ...
    for word in file_contents:
        if word in deny_list:
            print(f"ERROR -- Found in {file_location}")
            sys.exit(f"ERROR -- {word} found, and is not allowed.")


def character_scan(file_contents, file_location):
    if twenty := re.findall(
        "[^A-Z0-9][A][ACGIKNPRS][A-Z]{2}[A-Z0-9]{16}[^A-Z0-9]", file_contents
    ):
        for word in twenty:
            if word[1:-1] in allow_list:
                continue
            print(f"ERROR -- {word[1:-1]} Found in {file_location}")
...
def file_name_check(filename, file_location):
    if forty := re.findall(
        "[^a-zA-Z0-9/+=][a-zA-Z0-9/+=]{40}[^a-zA-Z0-9/+=]", filename
    ):
        if forty[0][-1] not in allow_list:
            print(f"ERROR -- {forty[0][:-1]} Found in {file_location}")
    ...
    for s in words:
        if snippet_start in s:
            ...
            if s in snippet_tags:
                ...
                print(f"ERROR -- Found in {file_location}")
                sys.exit(f"ERROR -- {s}'s matching start tag not found.") 

    if snippet_tags:
        print(f"ERROR -- Found in {file_location}")
        print(snippet_tags)
        sys.exit(f"ERROR -- {snippet_tags.pop()}'s matching end tag not found.")
...
def keyword_service_name(words, warn):
    ...
    if matching:
        contains_service_tag = True
    if not contains_service_tag and warn:
...
def keyword_language_check(words, warn):
    ...
    for language in languages:
        language_keyword = [s for s in words if f'keyword:[{language}]' in s]
        if language_keyword:
            contains_language_tag = True
            break
    if not contains_language_tag and warn:
...
def keyword_sdk_check(words, warn):
    ...
    for sdk in sdk_versions:
        sdkkeyword = [s for s in words if f'keyword:[{sdk}]']
        if sdkkeyword:
            contains_sdk_tag = True
            break
    if not contains_sdk_tag and warn:
