...
def verify_sample_files(root_path):
    ...
    for path, dirs, files in os.walk(sample_files_folder, topdown=True):
        for file_name in files:
            ...
            if file_name not in EXPECTED_SAMPLE_FILES:
                logger.error(
                    "File '%s' in %s was not found in the list of expected sample files. If this is a new sample file, add it to the EXPECTED_SAMPLE_FILES list in checkin_tests.py.",
                    file_name,
                    sample_files_folder,
                )
                error_count += 1
    ...
    for sample_file in EXPECTED_SAMPLE_FILES:
        if sample_file not in file_list:
            logger.error(
                "Expected sample file '%s' was not found in '%s'. If this file was intentionally removed, remove it from the EXPECTED_SAMPLE_FILES list in checkin_tests.py.",
                sample_file,
                sample_files_folder,
            )
            error_count += 1
